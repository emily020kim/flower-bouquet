...
const SunflowerPage = () => {
  ...
  const [sunflowerData, setSunflowerData] = useState<Sunflower | null>(null);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:5000/sunflower");
        const parsedData = await response.json();
        setSunflowerData(parsedData);
      } catch(error) {
  ...
  const adjustSunflowerForScreenSize = () => {
  ...
  const [roseScale, rosePosition, roseRotation] = adjustSunflowerForScreenSize();
            ...
            <div className='flex flex-col space-y-2 items-center justify-center bg-white rounded-md shadow-sm py-4 px-8 font-serif'>
              ...
              <div className='flex flex-col space-y-3 justify-start'>
                <h2 className='flex items-center text-lg font-medium'>
                  <GiFamilyHouse size={20} className='mr-2' />{sunflowerData.genus}
                </h2>
                <h3 className='flex items-center text-lg font-medium'>
                  <SiSpring size={15} className='mr-2 text-green-600' />{sunflowerData.season}
                </h3>
                <h3 className='flex items-center text-lg font-medium'>
                  <TbLineHeight size={20} className='mr-2' />{sunflowerData.height}
                </h3>
                <h3 className='flex items-center text-lg font-medium'>
                  <IoWaterSharp size={20} className='mr-2 text-blue-400' />{sunflowerData.water}
                </h3>
                <h3 className='flex items-center text-lg font-medium'>
                  <CiSun size={20} className='mr-2 text-yellow-400' />{sunflowerData.sunlight}
                </h3>
              </div>
            </div>
          )}
